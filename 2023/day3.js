...
...$.*....
.664.598..`;
let full = `...........441.................367................296........................................567..47.....45.................947.............
...
lines.forEach((line, i) => {
	...
	matches.forEach((match) => {
		numberGroups.push({
			...
			colEnd: match[0].length + match.index - 1,
		});
	});
});
...
let parts = numberGroups.filter((part) => {
	console.log(part);
	let indices = [];
	for (let i = part.colStart - 1; i <= part.colEnd + 1; i++) {
		indices.push([part.row - 1, i]);
		indices.push([part.row + 1, i]);
	}
	indices.push([part.row, part.colStart - 1]);
	indices.push([part.row, part.colEnd + 1]);
	indices = indices.filter(
		(index) =>
			index[0] >= 0 &&
			index[1] >= 0 &&
			index[0] < grid.length &&
			index[1] < grid[0].length
	);
	indices = indices.filter(
		(index) => grid[index[0]][index[1]] != "." && isNaN(grid[index[0]][index[1]])
	);
	indices.forEach((index) => {
		if (grid[index[0]][index[1]] == "*") {
			let g = gears.find((gear) => gear.row == index[0] && gear.col == index[1]);
			if (g) {
				g.num++;
				g.ratio *= part.num;
			} else {
				gears.push({ row: index[0], col: index[1], num: 1, ratio: part.num });
			}
		}
	});
	return indices.length > 0;
});
// console.log(parts.reduce((acc, cur) => acc + cur.num, 0));
console.log(
	gears.filter((gear) => gear.num == 2).reduce((acc, cur) => acc + cur.ratio, 0)
);
