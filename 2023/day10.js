...
let full = `F--FJ77F-L7---J-F777-F7|77F|-J-F|77|F-L-J-77-L7FL77-7|----.F7-|-|.7-.-FL-FF-FF77.|.F-F-.F|.7.F.7-F-77.J7|----7LL7F|7|-F-J-777-77.F-7-FL77FF|
...
let grid = full.split("\n").map((line) => line.split(""));
// console.log(grid);
let startRow = grid.findIndex((row) => row.includes("S"));
let startCol = grid[startRow].findIndex((cell) => cell == "S");
// console.log(startCol);
// console.log(startRow);

let steps = 0;
let current = { row: startRow, col: startCol, from: null };
let loop = [];
...
function move(pos) {
	let space = grid[pos.row][pos.col];
	loop.push([pos.row, pos.col, space, pos.from]);
	let dir = routes[space].find((d) => d != pos.from);
	switch (dir) {
		case "NORTH":
			pos.row--;
			pos.from = "SOUTH";
...
if (startRow > 0 && ["|", "7", "F"].includes(grid[startRow - 1][startCol])) {
	current.row = startRow - 1;
	current.col = startCol;
	current.from = "SOUTH";
} else if (
	startRow < grid.length - 1 &&
	["|", "L", "J"].includes(grid[startRow + 1][startCol])
) {
	current.row = startRow + 1;
	current.col = startCol;
	current.from = "NORTH";
} else if (startCol > 0 && ["-", "F", "L"].includes(grid[startRow][startCol - 1])) {
	current.row = startRow;
	current.col = startCol - 1;
	current.from = "EAST";
} else if (
	startCol < grid[0].length - 1 &&
	["-", "J", "7"].includes(grid[startRow][startCol + 1])
) {
	current.row = startRow;
	current.col = startCol + 1;
	current.from;
...
let startType = Object.keys(routes).find(
	(route) =>
		routes[route].includes(current.from) &&
		routes[route].includes(routes[loop[0][2]].find((d) => d != loop[0][3]))
);
loop.push([current.row, current.col, startType]);
// console.log(loop);
let trapped = [];
grid.forEach((row, i) =>
	row.forEach((cell, j) => {
		if (!loop.find((node) => node[0] == i && node[1] == j) && i > 0) {
			let walls = 0;
			let l2rJag = false;
			let r2lJag = false;
			for (let k = i - 1; k >= 0; k--) {
				// console.log(`row:${k} col:${j}`);
				let loopNode = loop.find((node) => node[0] == k && node[1] == j);

				if (loopNode) {
					if (loopNode[2] == "-") {
						walls++;
					} else if (loopNode[2] == "J") {
						l2rJag = true;
					} else if (loopNode[2] == "L") {
						r2lJag = true;
					} else if (loopNode[2] == "F" && l2rJag) {
						l2rJag = false;
						walls++;
					} else if (loopNode[2] == "7" && r2lJag) {
						r2lJag = false;
						walls++;
					} else if (loopNode[2] != "|") {
						r2lJag = false;
