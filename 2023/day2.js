...
let full = `Game 1: 1 red, 3 blue, 11 green; 1 blue, 5 red; 3 blue, 5 green, 13 red; 6 red, 1 blue, 4 green; 16 red, 12 green
...
let lines = full
	...
	.flat()
	.map((line) => line.split("; ").map((sub) => sub.split(", ").map((t) => t.split(" "))));
...
lines.forEach((game, i) => {
	if (game.every((round) => round.every((color) => color[0] <= limits[color[1]]))) {
		valids.push(i + 1);
	}
});

let maxes = lines.map((game) => {
	let max = { red: 0, blue: 0, green: 0 };
	game.forEach((round) => {
		round.forEach((color) => {
			if (color[0] > max[color[1]]) {
				max[color[1]] = Number(color[0]);
			}
		});
	});
	return max;
...
// console.log(valids.reduce((acc, cur) => acc + cur));
console.log(
	maxes
		.map((max) => Object.values(max).reduce((acc, cur) => acc * cur))
